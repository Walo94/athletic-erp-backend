...
router.get('/reportes/avance-dia', async (req, res) => {
  try {
    const response = await axios.get(`${REPORTES_API_URL}/reportes-api/inyeccion/avance-dia`, {
...
router.get('/reportes/avance-semana', async (req, res) => {
  try {
    const response = await axios.get(`${REPORTES_API_URL}/reportes-api/inyeccion/avance-semana`, {
...
router.get('/reportes/inventario-proceso', async (req, res) => {
  try {
    const response = await axios.get(`${REPORTES_API_URL}/reportes-api/inyeccion/inventario-proceso`, {
      responseType: 'arraybuffer'
    });
    res.setHeader('Content-Type', 'application/pdf');
    res.send(response.data);
